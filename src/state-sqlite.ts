 ...
 *
 * Multi-bot note: every bot adapter shares the central `chat_sdk_*` tables
 * (via getDb()). Without isolation their state collides — most visibly the
 * Chat SDK's per-message dedupe keys (`dedupe:<chat>:<msg>`), which omit any
 * bot identity, so two Telegram bots in one group would race on the same key
 * and only one would ever process each message. Pass a `namespace` (the
 * bot's channel_type) so each adapter's keys/threads are prefixed and stay
 * isolated. An empty namespace preserves the original (single-bot) behavior.
 */
...
export class SqliteStateAdapter implements StateAdapter {
  ...
  /** Prefix a key/thread id with the adapter namespace (no-op when unset). */
  private k(s: string): string {
    return this.ns ? `${this.ns}${s}` : s;
  }
  ...
  async get<T = unknown>(key: string): Promise<T | null> {
    this.cleanup();
    const row = this.db.prepare('SELECT value, expires_at FROM chat_sdk_kv WHERE key = ?').get(this.k(key)) as
  ...
  async setIfNotExists(key: string, value: unknown, ttlMs?: number): Promise<boolean> {
    const existing = this.db.prepare('SELECT expires_at FROM chat_sdk_kv WHERE key = ?').get(this.k(key)) as
  ...
  async acquireLock(threadId: string, ttlMs: number): Promise<Lock | null> {
    ...
    this.db.prepare('DELETE FROM chat_sdk_locks WHERE thread_id = ? AND expires_at < ?').run(this.k(threadId), now);
    const result = this.db
      .prepare('INSERT OR IGNORE INTO chat_sdk_locks (thread_id, token, expires_at) VALUES (?, ?, ?)')
      .run(this.k(threadId), token, expiresAt);
    if (result.changes === 0) return null;
    // Return the RAW threadId — releaseLock/extendLock re-apply the namespace.
  ...
  async appendToList(key: string, value: unknown, options?: { maxLength?: number; ttlMs?: number }): Promise<void> {
    ...
    const maxRow = this.db.prepare('SELECT MAX(idx) as maxIdx FROM chat_sdk_lists WHERE key = ?').get(this.k(key)) as
      | { maxIdx: number | null }
      | undefined;
    const nextIdx = (maxRow?.maxIdx ?? -1) + 1;
    this.db
      .prepare('INSERT INTO chat_sdk_lists (key, idx, value, expires_at) VALUES (?, ?, ?, ?)')
      .run(this.k(key), nextIdx, JSON.stringify(value), expiresAt);
    if (options?.maxLength) {
      const cutoff = nextIdx - options.maxLength;
      if (cutoff >= 0) {
        this.db.prepare('DELETE FROM chat_sdk_lists WHERE key = ? AND idx <= ?').run(this.k(key), cutoff);
      }
    }
  }
  ...
  async dequeue(threadId: string): Promise<QueueEntry | null> {
    ...
    const row = this.db
      ...
      .get(this.k(key)) as { idx: number; value: string } | undefined;
    if (!row) return null;
    this.db.prepare('DELETE FROM chat_sdk_lists WHERE key = ? AND idx = ?').run(this.k(key), row.idx);
    return JSON.parse(row.value) as QueueEntry;
  }

  async queueDepth(threadId: string): Promise<number> {
    const key = `queue:${threadId}`;
    const row = this.db.prepare('SELECT COUNT(*) as count FROM chat_sdk_lists WHERE key = ?').get(this.k(key)) as {
      count: number;
    };
    return row.count;
  }
