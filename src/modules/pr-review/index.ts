 ...
 * session-DB writes still come from the single host writer.
 *
 * Zero-config: no env vars required. The tick auto-discovers the reviewer
 * agent group from the DB (first group with a GitHub App identity). Set
 * PR_REVIEW_AGENT_GROUP_ID to override when multiple apps are configured.
 */
import { PR_REVIEW_AGENT_GROUP_ID, PR_REVIEW_COOLDOWN_MS, PR_REVIEW_STATUS_MESSAGING_GROUP_ID } from '../../config.js';
import { getAllGithubAppIdentities, getGithubAppForAgentGroup } from '../../db/github-apps.js';
...
export interface TickResult {
  status: 'ran' | 'misconfigured' | 'busy';
  dispatched?: number;
  reason?: string;
}

/** Resolve the reviewer agent group: explicit override → DB auto-discover. */
function resolveAgentGroupId(): string | null {
  if (PR_REVIEW_AGENT_GROUP_ID) return PR_REVIEW_AGENT_GROUP_ID;
  const identities = getAllGithubAppIdentities();
  return identities[0]?.agent_group_id ?? null;
}
...
export async function runPrReviewTick(): Promise<TickResult> {
  ...
  if (!agentGroupId) {
    return {
      ...
      reason: 'No GitHub App identity found — bind one with: ncl github-apps create',
    };
  }

  inFlight = true;
  try {
    const dispatched = await scanAndDispatch(agentGroupId);
