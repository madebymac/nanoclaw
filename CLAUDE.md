...
`scripts/onecli-reconcile-connections.sh` fixes this: for each instance it (1) moves any `app_connections` **and** `app_configs` row sitting in a foreign project into `proj-<instance>` (aligning `organization_id`), (2) grants the UI's `admin@localhost` user ownership of `org-<instance>`, (3) sets `proj-<instance>` as `admin@localhost`'s **default project** so the UI opens there and **future** UI connects land in `proj-<instance>` directly — the durable fix, not a band-aid — and (4) reaps the empty throwaway projects/orgs the UI already auto-created (only ones created by `admin@localhost` that hold no connections/configs and orgs with no projects + no real members; the target and anything with data are never touched). It is idempotent and best-effort, and `make deploy` runs it automatically after bouncing each gateway. Run it by hand after connecting an app in the UI:

Why the default project (step 3) matters: the local-mode UI bounces to `/create-org` (which 404s) whenever its NextAuth session has no `projectId`, and that comes from `findUserDefaultProject()`, which keys off a project's `created_by` — **not** org membership. So org ownership alone leaves `admin@localhost` stuck at `/create-org`; making `proj-<instance>` its `created_by` project is what lets the UI load and keeps connects in the right project. (Reaping `admin@localhost`'s personal project without this is exactly what 404'd `/overview` once.)
