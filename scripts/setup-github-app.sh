...
if [ "$WRITE_NOTE" = "1" ] && [ -n "$BOT_LOGIN" ]; then
  ...
  if [ ! -f "$NOTE" ] || ! grep -qF "$MARKER" "$NOTE"; then
    mkdir -p "groups/$FOLDER"
    {
      echo ""
      echo "$MARKER"
      echo "## GitHub identity — already authenticated, do NOT use the OneCLI connect flow"
      echo ""
      echo "**GitHub is ALREADY connected for you.** You act as the GitHub App bot"
      echo "**\`$BOT_LOGIN\`**. A short-lived installation token is injected into your"
      echo "environment as \`\$GH_TOKEN\` (also \`\$GITHUB_TOKEN\`), refreshed (~1h) every wake."
      echo ""
      echo "- **NEVER** tell the user GitHub \"isn't connected\" or hand them an OneCLI"
      echo "  \`…/connections?connect=github…\` URL. That flow does not apply to you — your"
      echo "  GitHub access is the App token below, not an OneCLI app-connection."
      echo "- **\`gh\` is NOT installed.** Use \`curl\` or \`git\` only."
      echo "- GitHub API: \`curl --noproxy '*' -H \"Authorization: Bearer \$GH_TOKEN\" -H \"Accept: application/vnd.github+json\" https://api.github.com/...\`"
      echo "  — the \`--noproxy '*'\` bypasses the OneCLI proxy and is the reliable path."
      echo "- List your accessible repos: \`curl --noproxy '*' -H \"Authorization: Bearer \$GH_TOKEN\" https://api.github.com/installation/repositories\`"
      echo "- git push: \`git push https://x-access-token:\$GH_TOKEN@github.com/<owner>/<repo>.git\`"
      echo "- Comments / PRs / reviews you post appear as \`$BOT_LOGIN\`, not the operator."
      echo "- Never print, echo, or commit the token."
    } >> "$NOTE"
