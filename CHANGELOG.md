...
- **Automatic PR review for un-reviewed open PRs.** A new opt-in host-side cron periodically scans every open PR across the review bot's GitHub App installation and, for any the bot hasn't reviewed yet, instructs the reviewer agent to review it (leave inline comments / approve). The "does this PR need review?" decision is made **programmatically** against the GitHub REST API — listing repos/PRs and checking for the bot's own reviews/comments — so **no AI tokens are spent** deciding; the agent is only woken when a PR actually needs review. A dispatch-tracking table (migration 017) keeps a PR from being re-instructed every tick while the agent's review is still in flight, with re-dispatch on new commits or after a cooldown. Enable with `PR_REVIEW_ENABLED=true` + `PR_REVIEW_AGENT_GROUP_ID=<reviewer agent group>` (which must have a `github_app_identity`); `PR_REVIEW_INTERVAL_MS` (default 60000) and `PR_REVIEW_COOLDOWN_MS` (default 1800000) tune cadence. See [docs/pr-review.md](docs/pr-review.md) and `src/modules/pr-review/`.
