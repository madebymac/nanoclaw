...
export type ProviderEvent =
  ...
  /**
   * Incremental assistant text as the model produces it. `text` is the
   * full accumulated assistant text so far for the current turn (not a
   * delta), so consumers don't have to reassemble. Providers that don't
   * stream may omit this entirely — the `result` event still carries the
   * final text and is authoritative.
   */
